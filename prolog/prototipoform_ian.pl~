...
obtener_acompannamientos(TiempoComida, Acompannamientos) :-
    ...
    ;
     ...
     % Preguntar cuántos acompañamientos desea el usuario (1 a 3)
    obtener_cantidad_acompannamientos(Cantidad),

    % Generar todas las combinaciones posibles con la cantidad seleccionada
    generar_combinaciones(Acompannamientos, Cantidad, Combinaciones).

% Predicado para generar todas las combinaciones posibles de una lista
generar_combinaciones(_, 0, [[]]).
generar_combinaciones(L, N, [Comb | Combs]) :-
    N > 0,
    N1 is N - 1,
    select(X, L, L1),
    Comb = [X | Comb1],
    generar_combinaciones(L1, N1, Combs),
    Comb = [_ | Comb1].

% Predicado para preguntar la cantidad de acompañamientos deseada (1 a 3)
obtener_cantidad_acompannamientos(Cantidad) :-
